...
class TUtils: NSObject {
    ...
    /**
     Write json data to file on local disk
     
     - parameter json:     JSON data
     - parameter fileName: file name
     
     - returns: true if saving json to file success
     */
    static func writeJSONToFile(json : NSDictionary, fileName : String) -> Bool {
        let documentDirectoryURL : NSString =  try! NSFileManager().URLForDirectory(.DocumentDirectory, inDomain: .UserDomainMask, appropriateForURL: nil, create: true).absoluteString as NSString
        let fileAtPath : NSString = documentDirectoryURL.stringByAppendingPathComponent(fileName)
        
        let jsonData : NSData = try! NSJSONSerialization.dataWithJSONObject(json, options: .PrettyPrinted)
        let jsonString : NSString = NSString(data: jsonData, encoding: NSUTF8StringEncoding)!
        
        
        
        return (jsonString.dataUsingEncoding(NSUTF8StringEncoding))!.writeToFile(fileAtPath as String, atomically: false)
    }
    
    /**
     Calculating height for text
     ...
     - returns: Height
     */
    static func heightForText(text : NSString, width : CGFloat, font : UIFont) -> CGFloat {
        if text.length > 0 {
            let frame : CGRect = text.boundingRectWithSize(CGSizeMake(width, CGFloat.max), options: .UsesLineFragmentOrigin, attributes: [NSFontAttributeName : font], context: nil)
